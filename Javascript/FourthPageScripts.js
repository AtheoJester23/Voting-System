...
document.querySelector('.ProceedEditPos').addEventListener('click', () => {
    CandidatePosition[3].Position = RnmTitlePos.value;
...
document.querySelector('.ProceedAddCandToPos').addEventListener('click', () => {
    FourthPageCandidates.push({Picture: AddImgFP.value, Name: AddNameFP.value, Year_Level: AddYLFP.value, Course: AddCourseFP.value, Votes: 0});
    SaveFPCand();
    window.location.reload();
})
...
function GoEditInfo(What){
    ...
    if(EdtInpCourseFP.value === "" && EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        ...
        FourthPageCandidates[What].Name = EdtInpNameFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpNameFP.value === "" && EdtInpYLFP.value === "" && EdtInpCourseFP.value === ""){
        FourthPageCandidates[What].Picture = EdtInpPicFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        FourthPageCandidates[What].Course = EdtInpCourseFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpCourseFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        FourthPageCandidates[What].Year_Level = EdtInpYLFP.value;
        SaveFPCand();
        window.location.reload();
    }
}
...
function ContinueRemovalCand(DeleteThis){
    FourthPageCandidates.splice(DeleteThis, 1);
    SaveFPCand();
    window.location.reload();
}
