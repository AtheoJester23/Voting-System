...
document.querySelector('.ProceedEditPos').addEventListener('click', () => {
    CandidatePosition[4].Position = RnmTitlePos.value;
...
document.querySelector('.ProceedAddCandToPos').addEventListener('click', () => {
    FifthPageCandidates.push({Picture: AddImgFP.value, Name: AddNameFP.value, Year_Level: AddYLFP.value, Course: AddCourseFP.value, Votes: 0});
    SaveFPCand();
    window.location.reload();
})
...
function GoEditInfo(What){
    ...
    if(EdtInpCourseFP.value === "" && EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        ...
        FifthPageCandidates[What].Name = EdtInpNameFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpNameFP.value === "" && EdtInpYLFP.value === "" && EdtInpCourseFP.value === ""){
        FifthPageCandidates[What].Picture = EdtInpPicFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        FifthPageCandidates[What].Course = EdtInpCourseFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpCourseFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        FifthPageCandidates[What].Year_Level = EdtInpYLFP.value;
        SaveFPCand();
        window.location.reload();
    }
}
...
function ContinueRemovalCand(DeleteThis){
    FifthPageCandidates.splice(DeleteThis, 1);
    SaveFPCand();
    window.location.reload();
}
