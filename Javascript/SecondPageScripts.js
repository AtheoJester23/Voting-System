...
document.querySelector('.ProceedEditPos').addEventListener('click', () => {
    CandidatePosition[1].Position = RnmTitlePos.value;
...
document.querySelector('.ProceedAddCandToPos').addEventListener('click', () => {
    SecondPageCandidates.push({Picture: AddImgFP.value, Name: AddNameFP.value, Year_Level: AddYLFP.value, Course: AddCourseFP.value, Votes: 0});
    SaveSPCand();
    window.location.reload();
})
...
function GoEditInfo(What){
    ...
    if(EdtInpCourseFP.value === "" && EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        ...
        SecondPageCandidates[What].Name = EdtInpNameFP.value;
        SaveSPCand();
        window.location.reload();
    }else if(EdtInpNameFP.value === "" && EdtInpYLFP.value === "" && EdtInpCourseFP.value === ""){
        SecondPageCandidates[What].Picture = EdtInpPicFP.value;
        SaveSPCand();
        window.location.reload();
    }else if(EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        SecondPageCandidates[What].Course = EdtInpCourseFP.value;
        SaveSPCand();
        window.location.reload();
    }else if(EdtInpCourseFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        SecondPageCandidates[What].Year_Level = EdtInpYLFP.value;
        SaveSPCand();
        window.location.reload();
    }
}
...
function ContinueRemovalCand(DeleteThis){
    SecondPageCandidates.splice(DeleteThis, 1);
    SaveSPCand();
    window.location.reload();
}
