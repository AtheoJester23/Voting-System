console.log("This is 6th page for voters")
...
console.log(SixthPageCandidates);
...
//ChangeThis
document.querySelector('.ThePosition').innerHTML = CandidatePosition[5].Position;


let Something = '';

//ChangeThis
SixthPageCandidates.forEach((Candidates, index) => {
    Something += `
...
function GetCandidateIndex(VoteCand){
    if(MonitorVoteCounting[5] === false){
        currentVote[5].Sixth = VoteCand;
        currentVoteState[5] = true;

        MonitorVoteCounting[5] = true;
        SaveMVC();
        SaveVoteState();
        SaveCandIndex();
        PlusVoteCount();
        window.location.href = "/Vote Page.html";
    }else if(MonitorVoteCounting[5] === true){
        //AddThis4
        currentVoteState[5] = false;
        MonitorVoteCounting[5] = false;
        //AddThis5
        SaveMVC();
        SaveVoteState();
        if(voteCounted > 0){
...
if(currentVote[5].Sixth < 10){
    if(MonitorVoteCounting[5] === true){
        if(currentVoteState[5] === true){
            document.querySelector(`.PageVoteBox${currentVote[5].Sixth}`).style.border = "Thin solid Green";
        }
    }else if(MonitorVoteCounting[5] === false){
        document.querySelector(`.PageVoteBox${currentVote[5].Sixth}`).style.border = "none";
    }
}