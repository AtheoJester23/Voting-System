console.log("This is 7th page for voters")
...
// voteCounted += 1;
console.log("The vote counted is: ")
...
//ChangeThis
document.querySelector('.ThePosition').innerHTML = CandidatePosition[6].Position;


let Something = '';

//ChangeThis
SeventhPageCandidates.forEach((Candidates, index) => {
    Something += `
...
function GetCandidateIndex(VoteCand){
    if(MonitorVoteCounting[6] === false){
        currentVote[6].Seventh = VoteCand;
        currentVoteState[6] = true;

        MonitorVoteCounting[6] = true;
        SaveMVC();
        SaveVoteState();
        SaveCandIndex();
        PlusVoteCount();
        window.location.href = "/Vote Page.html";
    }else if(MonitorVoteCounting[6] === true){
        //AddThis4
        currentVoteState[6] = false;
        MonitorVoteCounting[6] = false;
        //AddThis5
        SaveMVC();
        SaveVoteState();
        if(voteCounted > 0){
...
if(currentVote[6].Seventh < 10){
    if(MonitorVoteCounting[6] === true){
        if(currentVoteState[6] === true){
            document.querySelector(`.PageVoteBox${currentVote[6].Seventh}`).style.border = "Thin solid Green";
        }
    }else if(MonitorVoteCounting[6] === false){
        document.querySelector(`.PageVoteBox${currentVote[6].Seventh}`).style.border = "none";
    }
}