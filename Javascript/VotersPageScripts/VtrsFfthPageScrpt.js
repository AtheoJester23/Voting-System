console.log("This is 5th page for voters")
...
console.log(FifthPageCandidates);
...
//ChangeThis
document.querySelector('.ThePosition').innerHTML = CandidatePosition[4].Position;


let Something = '';

//ChangeThis
FifthPageCandidates.forEach((Candidates, index) => {
    Something += `
...
function GetCandidateIndex(VoteCand){
    if(MonitorVoteCounting[4] === false){
        currentVote[4].Fifth = VoteCand;
        currentVoteState[4] = true;

        MonitorVoteCounting[4] = true;
        SaveMVC();
        SaveVoteState();
        SaveCandIndex();
        PlusVoteCount();
        window.location.href = "/Vote Page.html";
    }else if(MonitorVoteCounting[4] === true){
        //AddThis4
        currentVoteState[4] = false;
        MonitorVoteCounting[4] = false;
        SaveMVC();
        SaveVoteState();
        if(voteCounted > 0){
            MinusVoteCount();
...
if(currentVote[4].Fifth < 10){
    if(MonitorVoteCounting[4] === true){
        if(currentVoteState[4] === true){
            document.querySelector(`.PageVoteBox${currentVote[4].Fifth}`).style.border = "Thin solid Green";
        }
    }else if(MonitorVoteCounting[4] === false){
        document.querySelector(`.PageVoteBox${currentVote[4].Fifth}`).style.border = "none";
    }
}