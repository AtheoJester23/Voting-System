...
document.querySelector('.ProceedEditPos').addEventListener('click', () => {
    CandidatePosition[5].Position = RnmTitlePos.value;
...
document.querySelector('.ProceedAddCandToPos').addEventListener('click', () => {
    SixthPageCandidates.push({Picture: AddImgFP.value, Name: AddNameFP.value, Year_Level: AddYLFP.value, Course: AddCourseFP.value, Votes: 0});
    SaveFPCand();
    window.location.reload();
})
...
function GoEditInfo(What){
    ...
    if(EdtInpCourseFP.value === "" && EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        ...
        SixthPageCandidates[What].Name = EdtInpNameFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpNameFP.value === "" && EdtInpYLFP.value === "" && EdtInpCourseFP.value === ""){
        SixthPageCandidates[What].Picture = EdtInpPicFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpYLFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        SixthPageCandidates[What].Course = EdtInpCourseFP.value;
        SaveFPCand();
        window.location.reload();
    }else if(EdtInpCourseFP.value === "" && EdtInpNameFP.value === "" && EdtInpPicFP.value === ""){
        SixthPageCandidates[What].Year_Level = EdtInpYLFP.value;
        SaveFPCand();
        window.location.reload();
    }
}
...
function ContinueRemovalCand(DeleteThis){
    SixthPageCandidates.splice(DeleteThis, 1);
    SaveFPCand();
    window.location.reload();
}
